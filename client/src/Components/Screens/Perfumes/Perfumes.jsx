...
import { setPerfumesFn,fetchProducts } from "../../../Redux/productReducer";
...
export const Perfumes = ()=>{
    ...
    const perfumesItems = useSelector((store) => store.productRed.perfumesArr);
    ...
    const getPost = async ()=>{
        ...
        try{
            const resultAction = await dispatch(fetchProducts("fragrances"));

            if(fetchProducts.fulfilled.match(resultAction))
            {
                dispatch(setPerfumesFn(resultAction.payload));
            }
    ...
    return (
        ...
        <h1 className="fs-1 mt-3 ms-5 pb-3 pt-2 product-page-title">Perfumes</h1>
        {
        ...
        <Container>
            ...
            <Row>
            {
                perfumesItems && perfumesItems.length>0 ? 
                
                
                    perfumesItems.map((item) => (
                        <>
                        <Col sm="4" >
